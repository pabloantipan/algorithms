...
class SimpleNode(BaseNode):
    ...
    def add_first_node(self, node):
        ...
        node.__next_node = self.__next_node
        node.__previous_node = self
        self.__next_node = node

    def replace_node(self, node):
        """Add a node and returns new reference."""
    ...
    def add_array_at_end(self, items):
        ...
        if last_node.__previous_node is None:
            self.information = copy_of_items.pop(0)
            last_node, _ = self.get_last()

        print(last_node)

        for item in copy_of_items:
            last_node.add_information(item)
